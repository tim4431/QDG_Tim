...
def run_opt(save_folder: str, grating_len: float, wg_width: float) -> None:
    ...
    wg_thickness = 220  # Tim

    sim_space = create_sim_space(  # Tim
        "sim_fg.gds",
        "sim_bg.gds",
    ...
    trans_list = create_transformations(
        obj, monitors, 50, 200, sim_space, min_feature=60
    )
...
def create_sim_space(
    ...
    stack = [
        ...
        optplan.GdsMaterialStackLayer(
            foreground=optplan.Material(mat_name="Air"),
    ...
    if visualize:
        ...
        eps_fg, eps_bg = get_fg_and_bg(context.get_object(simspace), wlen=1326)  # Tim
...
def create_objective(
    ...
    # o_p1 = objectives[3]
    # o_p2 = objectives[4]
    # ratio_1 = 0.8521437889662113
    # ratio_2 = 0.5272924240430485
    # obj = o_0 * 1 + o_m1 * ratio_1 + o_p1 * ratio_1 + o_m2 * ratio_2 + o_p2 * ratio_2
...
def view_opt(save_folder: str) -> None:
    ...
    log_tools.plot_monitor_data(log_df, monitor_descriptions, "./output.pdf")
